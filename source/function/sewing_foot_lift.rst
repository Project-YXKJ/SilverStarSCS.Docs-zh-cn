...
* **线迹中** ：反踩调速器至半后踏（:term:`POSITION -1`） 或者当 :option:`A14` 设置为1时自动抬起；
* **剪线之后** ：反踩调速器（:term:`POSITION -1` or :term:`POSITION -2`） 或者
  当 :option:`A15` 设置为1时自动抬起。
...
将参数 :option:`O06` 设置为1，压脚最大抬起时间由参数 :option:`O07` 决定。
